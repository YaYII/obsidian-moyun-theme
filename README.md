...
分组全部走主题令牌，因此**明暗两种模式自动成立**，字号也跟着分辨率一起放大 ——
不会再出现「图里的小字比正文还小」。图形比版心宽时容器横向滚动，不会右侧被裁掉。

Mermaid 会把默认配色内联进 SVG 内部的 ID 选择器样式里，主题用 `!important` 覆盖它；
构建期对此设有 `@important-budget` 预算机制（声明预算 + 写明理由，否则构建报错）。
...
- **视觉与计算样式验证**：用 Playwright 把主题渲染进 Obsidian 的真实 DOM，断言 47 项计算后样式（含「宽屏字号确实变大」「一行始终 46 字」等分辨率自适应实测）并输出明暗双模式截图
