...
- **行高与版心**：正文默认行高 1.8、版心 46rem（约 34 个汉字），这是中文长文阅读的舒适区。
...
六组设置：配色 / 中文排版 / 字体 / 行距与版心 / 界面细节 / 内容增强
...
```
34 个模块  →  node build.mjs  →  theme.css (422 KB)
```

- **构建期校验**：花括号配对、未定义令牌、原始色板越界引用、Style Settings 格式
- **模块冲突检查**：检测跨模块的「同一个属性有两个主人」，这是并行开发最容易埋的坑
- **视觉与计算样式验证**：用 Playwright 把主题渲染进 Obsidian 的真实 DOM，断言 28 项计算后样式并输出明暗双模式截图
